...
def view_in_listbox():
    ...
    for row in book_store_backend.view_all():
        list_box.insert(tk.END, row, print(row))# "END" inserts a new row at the end of the existing rows
        #print(row)
...
def update_command():
    book_store_backend.update(selected_tuple[0],ent_title.get(),ent_author.get(),ent_year.get(),ent_isbn.get())
    view_in_listbox()
        
window_main = tk.Tk()
window_main.wm_title("Book Store 2.0")
