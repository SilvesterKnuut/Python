...
hostFile_path =r"C:\Windows\System32\drivers\etc\hosts" 
redirect = "127.0.0.1"
...
while True:
    ...
    if dt(dt.now().year,dt.now().month,dt.now().day,9) < dt.now() < dt(dt.now().year,dt.now().month,dt.now().day,17):
        with open(hostFile_path, 'r+') as file: # "r+" means read and append if there was only r then only 4 lines would be writtent to the file
            content=file.read()
            file.seek(0,2)
